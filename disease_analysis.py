...
def predict_zero_shot(
    ...
    Args:
        images: 图像张量或列表
        models: 模型字典，包含resnet, image_projector, text_model, text_projector, tokenizer
        disease_list: 疾病列表
        top_k: 返回前k个预测
        prompts: 提示模板字典
    ...
    with torch.no_grad():
        ...
        # 获取图像嵌入
        image_embeddings = resnet(images)
        image_embeddings = image_embeddings.view(image_embeddings.size(0), -1)
        
        # 投影到共享空间
        image_features = image_projector(image_embeddings)
        image_features = F.normalize(image_features, dim=-1)
        
        # 使用增强的提示或基本提示获取文本特征
        if use_enhanced_prompts and prompts:
            try:
                # 使用增强的文本特征
                text_features = torch.cat([
                    get_enhanced_text_features(
                        disease, prompts, models['tokenizer'], models['text_model'], models['text_projector'], DEVICE
                ...
                text_features = get_text_features(
                    disease_list, models['tokenizer'], models['text_model'], models['text_projector'], DEVICE
                )
        else:
            # 使用基本文本特征
            text_features = get_text_features(
